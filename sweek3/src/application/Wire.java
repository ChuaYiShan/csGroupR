...
public class Wire extends AnchorPane {
	...
	private void initialize() {
		...
		node_link.controlX1Property().bind(Bindings.add(node_link.startXProperty(), mControlOffsetX.multiply(mControlDirectionX1)));
		
		node_link.controlX2Property().bind(Bindings.add(node_link.endXProperty(), mControlOffsetX.multiply(mControlDirectionX2)));
		
		node_link.controlY1Property().bind(Bindings.add(node_link.startYProperty(), mControlOffsetY.multiply(mControlDirectionY1)));
		
		node_link.controlY2Property().bind(Bindings.add(node_link.endYProperty(), mControlOffsetY.multiply(mControlDirectionY2)));
	}
	...
	public void bindEnds(Component source, Component target) {
		...
		if (target.getConnectedComponentList().contains(source)){
			
			// right border of source
			...
			// left border of target
			node_link.endXProperty().bind(
					Bindings.add(target.layoutXProperty(), 0));
			
			node_link.endYProperty().bind(
					Bindings.add(target.layoutYProperty(), (target.getWidth() / 2.0) + 10));
